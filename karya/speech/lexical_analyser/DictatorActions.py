...
special_chars = dict(question_mark=("?", "question mark"),
                     ...
                     full_stop=(".", "full stop", "fullstop", "dot", "period"), comma=(",", "comma"),
                     space_bar=(" ", 'space bar', 'spacebar'),
...
def _handle_put(txt: str):
    if "digit" in txt:
        # digit input
        return _find_digit(txt)
    return _find_special(txt)
...
def _find_special(txt_with_special: str):
    ...
    if "times" not in txt_with_special and "time" not in txt_with_special:
        ...
        times = get_number(txt_with_special)
    # looking for specials
    for char in special_chars:
        for string in special_chars[char]:
